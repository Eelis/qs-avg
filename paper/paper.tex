...
A completely different approach to type-theoretic analysis of computational complexity is to devise a special-purpose type theory in which the types of terms include some form of complexity guarantees. This is the approach taken in \cite{constable}, for example. However, as far as we can see, this method is not suitable for the formalization of a traditional complexity proof in which an algorithm is first defined and then separately analyzed. We are not aware of practical techniques usable for this purpose apart from the one we present in this paper.
...
Note that the Quicksort definition in section \ref{embed} could have been implemented with a |partition| pass as well, which might well have made the worst-case proof even simpler. We chose not to do this, in order to emphasize that the properties the average-case proof demands of the algorithm rule out the naive but familiar implementation using |filter| passes.
