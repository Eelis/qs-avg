...
\begin{proof}
  The proof is by induction on |qs|. For an empty input list, we have |cost (qs_SP nil) = cost (ret nil) = 0 <= sqrd (length l)|. For a non-empty input list |h :: t|, the cost decomposes into
  \begin{code}
...
\begin{code}
  forall l, expec cost (qs_NDP l) <= 2 * length l * (1 + log2ceil (length l)),
\end{code}
