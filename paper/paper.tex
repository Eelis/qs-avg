...
programming language, in the form of Coq's CIC \cite{coq'art}. However, just
...
programming and monad laws, see \cite{wadler95monads}.
...
For reasons that will become clear in later sections, we construct the monad with which we will instantiate the above definitions using a monad transformer \cite{monadtrans} |MMT| (for ``monoid monad transformer''), which piggybacks a monoid onto an existing monad by pairing.
...
We now use |MMT| to piggyback the additive monoid structure on |nat| onto the identity monad, and lift elementwise comparison into the resulting monad, which we call |SP| (for ``simply-profiled'').
\begin{code}
...
\begin{code}
  ...
  Notation {-"``"-}wrt m,{-"\ "-} f =O( g ){-"\text{''''}"-} := bigO m f g.
\end{code}
We now state the main theorem.
\begin{code}
  Theorem qs_avg: wrt length, expec cost fcmp qs_NDP =O(fun n => INR (n * log2ceil n)).
\end{code}
...
The right-hand inequality is a strictly numerical affair, requiring a bit of analysis involving the harmonic series. As stated before, this part of the proof 
    was 
%%% could be 
fairly directly transcribed from the paper proof, with few complications and additions, and so we will not discuss it.

The left inequality is the challenging one. To bring it closer to the index summation, we first write |l| on the left-hand side as |map (nth (sort l)) li|, where |sort| may be any sorting function (including |qs| itself), and where |li| is a permutation of |[0...n-1]| such that |map (nth (sort l)) li = l| (such an |li| can easily be proven to exist).
...
\end{code}
The function |qs_U| operates directly on lists of indices into |sort l|. Comparison of indices is defined by comparison of the %%%|T| 
...
We now distinguish between five different cases that can occur for the nondeterministically picked |pivot| (which, because we are in the |U| monad, is an index). It can either be less than |i|, equal to |i|, between |i| and |j|, equal to |j|, or greater than |j|. Each case occurs a certain number of times, and has an associated expected number of |(i, j)| comparisons (coming either from the |map_fst| term representing the |partition| pass, or from the two recursive |qs_U| calls). To represent this split, we first rewrite the right-hand side of the inequality to\[\frac{\Varid{ecc}\ i\ j * (i - b) + 1 + 0 + 1 + \Varid{ecc}\ i\ j * (b + n - j)}{n}\ .\]
...
at the end of section~\ref{reduction} on page \pageref{monoid_expec_zero_lem}). Hence, the first |monoid_expec| term vanishes%%%
%%% as well
, leaving
\begin{code}
monoid_expec (count (i, j))
...
%%% the shallowness of the embedding 
...
%%% framework for 
    approach 
%%% to worst- and average-case complexity that 
we have developed. 
...
% LocalWords:  Nijmegen GL Quicksort Coq todo monadically Hurd Audebaud Paulin
% LocalWords:  Mohring na ve Coq's pCIC ret Haskell's liftM forall filterM bool
% LocalWords:  parameterizing le gt negb Fixpoint qs Quicksort's customized MMT
% LocalWords:  specialized elementwise idealized monoid fcmp ab snd mult fst SP
% LocalWords:  nat NatAddMonoid IdMonad Eval infixbind sqrd flt Eq cmp const ne
% LocalWords:  upp avg ary expec NDP MonoidMonadTrans NeTreeMonad bigO INR ceil
% LocalWords:  IJ ecc li fr NotIn generalizing ca cb foo parameterized
